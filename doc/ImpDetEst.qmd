...
tmp$CapHist <- with(tmp, paste("DD", Rear, sep = "."))
nACH <- nrow(tmp)
cDAFWSColonyID <- sprintf("DA.%04d", 1:nACH)
tmp$FWSColonyID <- cDAFWSColonyID
tCapHist <- data.frame(CapHist %>%
                       select(-CapHist))
colnames(tCapHist) <- colnames(tmp)

CapHistAug <- bind_rows(tCapHist, tmp)
CapHistAug$Year <- factor(CapHistAug$Year, cYear)
CapHistAug$Str <- factor(CapHistAug$Str, cDFStr)
Cmb2FS <- nStr + as.numeric(CapHistAug$Str)
rm(tmp)
...
tMltObsr <- matrix(
  ...
  ncol = length(cCHCol),
  dimnames = list(CapHistAug$CapHist, cCHCol)
)
tMltObsr[, ] <- as.matrix(CapHistAug[, c("01", "10", "11", "00")])
...
# Z[FWSColonyID]~bernoulli(1|Str);
ColObs <- SmpColObs %>%
  mutate(FS = factor(paste(Year, Frame, Str, sep = "."), cFS),
         GndNest = ifelse(Obsr == "Gnd", Nest, NA)) %>%
  group_by(FWSColonyID, FS) %>%
  summarize(
    Nest = pmax(0, max(Nest, na.rm = TRUE)),
    Act = pmax(0, max(Act, na.rm = TRUE)),
...
# For now all the ground were in sample for some reason.
# Idx <- SmpColObs[SmpColObs$FWSColonyID %in% GroundObs[GroundObs$InSmp, "FWSColonyID"], "FWSColonyID"]
# Now none of the ground IDs match the sample IDs.  Why?!
# Idx <- SmpColObs[SmpColObs$FWSColonyID %in% GroundObs[, "FWSColonyID"], "FWSColonyID"]
# ColObs[!ColObs$FWSColonyID %in% Idx, "GndNest"] <- NA

# Could get the nest that had "Dirt"
ColObs[ColObs$GndNest == 0, "GndNest"] <- NA

Col2FS <- as.integer(ColObs$FS)
nLstCol <- nrow(filter(ColObs, grepl("List", ColObs$FS)))
nNewCol <- nrow(filter(ColObs, grepl("Area", ColObs$FS)))
nCol <- nrow(ColObs)
...
Data <- list(
  ...
  nNewCol = nNewCol,
  nObsr = nObsr,
  nColObs = nrow(SmpColObs), # nLstCol + nNewCol + nACH
  nest = SmpColObs$Nest,
  act = SmpColObs$Act,
  Obs2Col = Obs2Col,
  Obs2Obsr = Obs2Obsr,
...
DebugMe <- FALSE
 ...
 # JAGS model
 BUGSMdl <-
   paste(readLines(here::here(
     paste("extdata/Models/", cMdl, ".mdl", sep =
             "")
   )), collapse = "\n")
