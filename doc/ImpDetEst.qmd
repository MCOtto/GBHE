...
tmp$ColID <- cDAColID
tOSCmb <- data.frame(OSCmb %>%
                       select(-CapHist))
colnames(tOSCmb) <- colnames(tmp)

OSCmb <- bind_rows(tOSCmb, tmp)
...
```{r AugmentedColonies} 
cColID <- sort(unique(c(RecObs$FWSColonyID, Colony$FWSColonyID)))
cColLvl <- sort(unique(c(cColID, cDAColID)))
Cmb2Col <- as.numeric(factor(OSCmb$ColID, cColLvl))
sum(duplicated(OSCmb$ColID)) # the new colonies

# Only used for observer detection, not seat detection.
Grp <- rbind(match(OSCmb$Front, cObsr), match(OSCmb$Rear, cObsr))
...
# Colony indicator to match number of nests in colonies
 ...
 tToStr <- rbind(cbind(0, diag(nStr)), cbind(1, diag(nStr)))
 ToStr <-  rbind(cbind(0, tToStr), cbind(1, tToStr))
