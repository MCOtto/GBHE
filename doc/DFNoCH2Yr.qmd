---
title: "GBHE 2015 Dual-Frame Survey Estimation with `DFNoCH` Model`
author: "Mark C. Otto"
date: "14 April 2025"
echo: false
eval: true
...
Fit the model in the 2018 paper,  `DFNoCH` Model`
...
InitFcn <- function(iChain) {
  ...
  ENest[SmpColYr2ColYr] <- tNest[SmpColYr2ColYr]
  all(Data$nest <= ENest[SmpColYr2ColYr])
  
  #If not consistent, check that the colony are the maximum of the Nest
  # Data$nest[which(SmpColYr2ColYr == 220)]
  ...
  # Give the are some chance of having nests
  lnNpC <- ifelse(NestSmry$NestPerCol == 0, log(1.1), log(NestSmry$NestPerCol))
