...
author: "Mark C. Otto 1 October 2024"
echo: false
eval: true
warning: false
error: false
...
```{r New2015Colonies}
New2015 <- RecObs %>%
  filter(Year == 2015 & stringr::str_detect(FWSColonyID, "new")) %>%
  mutate(
    nNest = pmax(fNest, rNest, na.rm = TRUE),
    nActive = pmax(fAct, rAct, na.rm = TRUE)
  ) %>%
  dplyr::select(Year, FWSColonyID, Str, PlotID, Lng, Lat, nNest, nActive)

write.csv(New2015, file = here::here("extdata/New2015.csv"), na = "", row.names = FALSE)


```

```{r, KnownNestPlots}
# I use n_distinct but the colony ids are unique
YrStrCol <- Colony %>%
  group_by(Year, Str) %>%
...
List for all list and new colonies, different from the `AllColonySF` layer.
...
AllCol <- bind_rows(
  Colony %>%
    ...
    dplyr::select(FWSColonyID, FS, Year, Str),
  RecObsArea %>%
...
# 2015 includes the new colonies found
AllCol %>%
  group_by(Year) %>%
  summarize(nCol = n())
...
Don't use the counts in a random effects Poisson model to estimate the stratum rates. Let the proportion of active colonies be Act\[s,o\] == 0.  Note the `pAct` here has not been how I calculated the active nests, I use the number of colonies as the demoninator.
...
knitr::kable(LstEst[, c("Year", "Str", "pOcc", "NestMn", "ActMn", "pAct")], digits = 2, caption = "Simple List Frame Summaries")
...
tSmpCol <- SmpColObs %>%
  ...
  dplyr::select(Year, FWSColonyID, Frame, Str, FS, InSmp)
...
NestPerCol <- tibble::tibble(
  ...
  separate(FS, into = c("Year", "Frame", "Str"))
...
ActSmry <- inner_join(
  bind_rows(
    LstSmry %>%
      mutate(Year = as.character(Year), Frame = "List") %>%
      dplyr::select(Year, Frame, Str, nColony = nLstAct, nColonySD = nLstActSD),
    NewSmry %>%
      mutate(
        Year = as.character(Year),
        Frame = "Area",
