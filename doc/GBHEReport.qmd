...
author: "Mark C. Otto 30 October 2024"
echo: false
eval: true
warning: false
error: false
...
# Make a year-frame-stratum levels
tYFS <- expand_grid(Year = cYear, Frame = cFrame, Str = cDFStr) %>%
  mutate(YFS = paste(Year, Frame, Str, sep = "."))
cYFS <- tYFS$YFS
```
...
GRTSSmpYFS <- tGRTSSmp %>%
  ...
  arrange(factor(Frame, cFrame), Year, factor(Str, cDFStr))
...
for (iYrStr in 1:nrow(LstSmry)) {
  ...
  tBeta <- Beta(nu = 1 + LstSmry[iYrStr, "nNestCol"], nup = 1 + LstSmry[iYrStr, "nSmpCol"] - LstSmry[iYrStr, "nNestCol"])
...
knitr::kable(LstSmry[, c("Year",
                         ...
                         "pNestColSD",
                         "nEstNestCol",
                         "nEstNestColSD")], digits = 2, caption = "List Frame Nest Summary")
...
knitr::kable(LstSmry[, c("Year",
                         ...
                         "pActColSD",
                         "nEstActCol",
                         "nEstActColSD")], digits = 2, caption = "List Frame Active Nest Summary")
rm(tLstSmry)
...
tMdlMtx <- model.matrix  ( ~ YrStr - 1, data = NewObsYS)
MdlMtx <- diag(NewObsYS$KMSq) %*% tMdlMtx

NewColMdl <- lm(nNestCol ~ MdlMtx - 1, data = NewObsYS)
summary(NewColMdl)
NewColGLM <- glm(nNestCol ~ tMdlMtx - 1, 
                 family = poisson, weight = log(NewObsYS$KMSq),
                 data = NewObsYS)
summary(NewColMdl)
summary(NewColGLM)
tAIC <- AIC(NewColMdl, NewColGLM)
tAIC$AIC <-  tAIC$AIC- c(0, sum(log(NewObsYS$KMSq)))
tAIC
...
```{r EstNewAct}
NewActColMdl <- lm(nActCol ~ MdlMtx - 1, data = NewObsYS)
...
summary(NestMdlFS)
AIC(NestMdl0, NestMdlStr, NestMdlYrStr, NestMdlFS)
...
# Use the YrStr
AIC(ActMdl0, ActMdlStr, ActMdlYrStr, ActMdlFS)

# MdlFS estimate and SD
cCoef <- stringr::str_replace(names(coefficients(ActMdlFS)), "YFS", "")
FSEst <- rep(0, length = length(cYFS))
names(FSEst) <- cYFS
...
cObsr <- unique(as.character(AllObs$Obsr))
...
CHObs <- RecObs %>%
  ...
  select(Year, Str, PlotID, FWSColonyID, Front, Rear, AOU, CapHist, nNest)
...
CapHist <- CHObs %>%
  # Frame, removed
  dplyr::select(Year, Str, PlotID, FWSColonyID, Front, Rear, AOU, CapHist) %>%
  mutate(
    Str = as.character(Str),
    # is a new nest?
  ...
  # Frame, removed
  group_by(Year, Str, Front, Rear, FWSColonyID, FRObsr, CH) %>%
  ...
  dplyr::select(Year, # Frame,
                Str, # inCNA,
                Front, Rear, FWSColonyID, FRObsr, `01`, `10`, `11`, `00`)
...
# Est
DetPN  <- bind_cols(
  Year = rep(cYear, each = 2),
  Seat = rep(cSeat[1:2], times = 2),
  pDet = round(100 * Est$par, 0),
  SD = round(100 * sqrt(diag(solve(Est$hessian))), 1),
  N = rep(currentN, each = 2)
)
knitr::kable(DetPN, digits = 1, caption = "Front-Rear Observer Detection Estimates")
