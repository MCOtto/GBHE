...
# single variables to `ori/MetaVars.RDump`.  Use `lWrite = FALSE` to debug and test.
lWrite <- FALSE
...
if(lWrite) {
  dump(
    ...
    file = here::here("ori/MetaVars.RDump")
  )
}
...
# Is in UTM 19N
if (lFromOri) {
  tColony2015 <- read_csv(here::here("ori/Colony2015.csv"), col_types = "c-iiidd")
} else {
...
if (lFromOri) {
  tRC2XY <- read_csv(here::here("ori/Colony.csv"), col_types = "c-iiidd")
} else {
...
2023 is a spatial file but 2015 was a dataframe.  How does that work?
...
if (lWrite) {
  write_csv(AllColony, here::here("data/AllColony.csv"), na = "")
...
# MCO 7 Oct 24: extracting data/RecObs and getting
# dataRecObs <- read_csv(here::here("data/RecObs.csv"), col_types = "-ic-c----ddc--ciiii")
#                       col_types = "-ic-c----ddc--ciiii") %>%
dataRecObs <- read_csv("~/Profesional/GBHE.2023/extdata/2023/RecObs.csv",
                       col_types = "-i------cc-dd---ciiii----") %>%
  mutate(
    FWSColonyID = sub("(ND|SC|Co)\\.", "", sub("new", "ME", FWSNestID)),
    nNest = pmax(0, fNest, rNest, na.rm = TRUE),
    nActive = pmax(0, fAct, rAct, na.rm = TRUE)
  ) %>%
  filter(!is.na(FWSColonyID)) %>%
  group_by(Yr, FWSColonyID, Str, PlotID) %>%
  summarize(
    Lng = mean(Lng),
    Lat = mean(Lat),
    nNest = max(nNest, na.rm = TRUE),
    nActive = max(nActive, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  select(Year = Yr, FWSColonyID, Str, PlotID, Lng, Lat, nNest, nActive)

addCol <- anti_join(dataRecObs, Colony2, by = c("Year", "FWSColonyID"))

Colony2 <- bind_rows(Colony, dataRecObs) %>%
  group_by(Year, FWSColonyID, Str, PlotID) %>%
  summarize(
    Lng = mean(Lng),
    Lat = mean(Lat),
    nNest = max(0, nNest, na.rm = TRUE),
    nActive = max(0, nActive, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  select(Year, FWSColonyID, Str, PlotID, Lng, Lat, nNest, nActive) %>%
  arrange(Year, FWSColonyID)

Colony2 <- bind_rows(
  Colony %>%
    filter(FWSColonyID == "ME.0854") %>%
    mutate(Year = 2015),
  Colony
) %>%
  arrange(Year, FWSColonyID)
...
if(lWrite) {
  readr::write_csv(Colony,
            file = here::here("data/Colony.csv"),
            na = "")
}
# Need to update ColonySF too!!

# Update recobs with the colony info
RecObs2 <- left_join(
  RecObs,
  Colony %>%
    select(
  ...
  mutate(
    ...
    FWSColonyID = ifelse(AOU != "GBHE", NA, FWSColonyID)
  )  %>%
  # filter(!is.na(AOU)) %>%
  select(
    Obs_PK,
...
if(lWrite) {
  write_csv(RecObs2,
            file = here::here("data/RecObs.csv"),
            na = "")
}
